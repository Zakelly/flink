...
public class FsSegmentDataInputStream extends FSDataInputStreamWrapper
    ...
    public FsSegmentDataInputStream(
        ...
        this.curPosInFile = startingPosition;
    }

    @Override
    public int read() throws IOException {
        if (curPosInFile >= endingPosition) {
            return -1;
        }
    ...
    public int read(byte[] b, int off, int len) throws IOException {
        ...
        int n = (int) Math.min(Integer.MAX_VALUE, Math.min(len, (endingPosition - curPosInFile)));
        if (n == 0) {
            return -1;
        }
        int ret = inputStream.read(b, off, n);
        if (ret < 0) {
            endingPosition = curPosInFile;
            return -1;
        }
        curPosInFile += ret;
        return ret;
    ...
    public void seek(long desired) throws IOException {
        ...
        curPosInFile = desired;
    }

    @Override
    public long getPos() throws IOException {
        return curPosInFile - startingPosition;
    }

    @Override
    public long skip(long n) throws IOException {
        long len = Math.min(n, endingPosition - curPosInFile);
        len = Math.min(len, inputStream.skip(n));
        curPosInFile += len;
        return len;
    }
    ...
    public synchronized void reset() throws IOException {
        ...
        inputStream.seek(markPosition);
        curPosInFile = markPosition;
    }
