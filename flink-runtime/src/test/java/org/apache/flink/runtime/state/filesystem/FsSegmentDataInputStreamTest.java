...
public class FsSegmentDataInputStreamTest {
    ...
    public void testReadFromFileSegments() throws IOException {
        ...
        assertEquals(segmentSize, readLen);
        ...
        assertEquals(seekPos, inputStream.getPos());
        readLen = inputStream.read(readResult);
        assertEquals(readBufferSize, readLen);
        assertBytesContentEqual(fileContent, startPos + seekPos, readLen, readResult);
        assertEquals(seekPos + readBufferSize, inputStream.getPos());
        // current relative position is (74 + 32 = 106)
        // reading another 32 bytes will cross the segment boundary
        assertEquals(segmentSize - seekPos - readBufferSize, inputStream.read(readResult));
        assertEquals(-1, inputStream.read());
        assertEquals(-1, inputStream.read(new byte[10]));
        assertEquals(-1, inputStream.read(new byte[10], 0, 1));
    }
    ...
    private void assertBytesContentEqual(
        ...
        assertEquals(sizeInExpected, actual.length);
        ...
        assertArrayEquals(expectedSegment, actual);
    }
}
